...
Instance1=$(aws ec2 run-instances --image-id $Image_ID --count 1 --instance-type t2.micro --key-name keyPair --security-group-ids $Security_group --subnet-id $Net1 --query 'Instances[-1].[InstanceId]' --output text) 
Instance2=$(aws ec2 run-instances --image-id $Image_ID --count 1 --instance-type t2.micro --key-name keyPair --security-group-ids $Security_group --subnet-id $Net2 --query 'Instances[-1].[InstanceId]' --output text)
echo $Security_group
echo $Image_ID
echo $Instance1
echo $Instance2